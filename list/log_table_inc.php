...
function table_ultra_simple($uid){
	global $db,$env_rows,$INDEX,$JOBLIST,$RACELIST,$FACENAME,$PLOF_SCRIPT,$ML_SCRIPT;
	//var_dump($uid);
	$env_anon_mode = $env_rows["anon_mode"];

	$sql = "select * from LOG_USER_DATA where LOG_USER_DATA.uid = '$uid'";
	$result = $db->query($sql);
	...
	if($members == "0"){
		echo "<TR class=\"table_title\"><TH>ユーザー未登録</TH></TR>";
                  ...
                  ?><TR class="table_title">
					  ...
					  <?php
					//sort
					$sql = log_sql_sort($uid);
					$result = $db->query($sql);
					$chk = $result->numRows();
					if(!$chk){
						echo "<TR><TD colspan=6>これより先のログはありません。</TD></TR>";
					}
					...
					while( $main_rows = $result->fetchRow(DB_FETCHMODE_ASSOC) ){
                      ...
                      <TR>
                        ...
                        <TD align="center" class="row<?php echo $row_c; ?>"><img src="../face/<?php echo $user_faceid; ?>.gif" alt="<?php echo "$RACELIST[$user_race] [$user_face_name]"; ?>"></TD>
                        <TD align="center" class="row<?php echo $row_c; ?>"><?php echo "$user_name"; ?></TD>
...
function table_simple($uid){
	global $db,$env_rows,$INDEX,$JOBLIST,$PLOF_SCRIPT,$ML_SCRIPT;
...
function table_job($uid){
	global $db,$env_rows,$INDEX,$JOBLIST,$COLOR,$PLOF_SCRIPT,$ML_SCRIPT,$JOB_MAX;
	...
	if($table==3){
		...
		echo "<td class='color9' nowrap>Lv100～</td>\n";
		echo "</tr>";
	} else {
		...
		echo "<td>&nbsp;&nbsp;</td><td class=row1 nowrap>&nbsp;support job&nbsp;</td></tr>\n";
                  ...
                  ?></td></TBODY></TABLE></TBODY></TABLE><TABLE border="0" width="100%" cellpadding="2" cellspacing="1" class="forumline">
					...
					$chk = $result->numRows();
					if(!$chk){
						echo "<TR><TD colspan=12>これより先のログはありません。</TD></TR>";
					}
					//データをwhileで回す
					while( $main_rows = $result->fetchRow(DB_FETCHMODE_ASSOC) ){
                      ...
                      <TR>
                        ...
                        <TD rowspan=3 align="center" class="row<?php echo $row_c; ?>"><?php echo "$user_name"; ?></TD>
...
function table_prod_item($uid){
	global $db,$env_rows,$INDEX,$COLOR,$PLOF_SCRIPT,$ML_SCRIPT,$PROD_MAX,$PLODLIST;
...
function table_prod_skill($uid){
	global $db,$env_rows,$INDEX,$COLOR,$PLOF_SCRIPT,$ML_SCRIPT,$SKILLLIST;
	...
	echo "<td align=right><table cellpadding=0 cellspacing=0 border=0>";
		...
		echo "</tr>";
                  ?></td></TBODY></TABLE></TBODY></TABLE><TABLE border="0" width="100%" cellpadding="2" cellspacing="1" class="forumline">
					...
					$chk = $result->numRows();
					if(!$chk){
						echo "<TR><TD colspan=11>これより先のログはありません。</TD></TR>";
					}
					//データをwhileで回す
					while( $main_rows = $result->fetchRow(DB_FETCHMODE_ASSOC) ){
                      ...
                      <TR class=tx10>
                        ...
                        <TD rowspan=4 align="center" class="row<?php echo $row_c; ?>"><?php echo "$user_name"; ?></TD>
						<?php
...
function table_stat($uid){
	global $db,$env_rows,$INDEX,$FACENAME,$RELMLIST,$RACELIST,$SIZELIST,$PLOF_SCRIPT,$ML_SCRIPT;
	...
	if($members == "0"){
		echo "<TR class=\"table_title\"><TH>ユーザー未登録</TH></TR>";
                  ...
                  ?><TR class="table_title">
					...
					$chk = $result->numRows();
					if(!$chk){
						echo "<TR><TD colspan=9>これより先のログはありません。</TD></TR>";
					}
					//データをwhileで回す
					while( $main_rows = $result->fetchRow(DB_FETCHMODE_ASSOC) ){
...
function table_full($uid){
	global $db,$env_rows,$INDEX,$COLOR,$PLOF_SCRIPT,$ML_SCRIPT,$SKILLLIST,$PROD_MAX,$PLODLIST,$FACENAME,$RELMLIST,$RACELIST,$SIZELIST,$JOBLIST;
